...
export default defineCommand({
  ...
  description: "Search for American Records for a given age and gender",
  options: {
  ...
  handler: async ({ flags, positional }) => {
    ...
    const sortedResults = [...results].sort((a: Record, b: Record) => {
      const aWeightClass = a.weightClass.match(/^(\d+)(\+)?kg$/);
      const bWeightClass = b.weightClass.match(/^(\d+)(\+)?kg$/);
